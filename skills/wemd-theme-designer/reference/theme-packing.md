...
1. 运行 `node scripts/pack-theme.cjs <theme-name>` 生成中间文件到 `output/theme-package/`
   - 打包前自动执行 `scripts/validate-css-selectors.mjs` 校验 CSS 选择器与嵌套 var，校验失败则中止打包
2. 执行 PowerShell 命令打包：
   ```powershell
   Compress-Archive -Path "./manifest.json","./brand.md","./styles" -DestinationPath "../{theme-name}.wemd-theme" -Force
   ```
3. 使用 `node scripts/validate-theme.cjs` 验证 manifest.json 规范性

## 打包脚本 (`scripts/pack-theme.cjs`)

核心流程：

1. **读取输入**：`BrandVisualTheme.json` + `output/css/{theme-name}.css`
...
3. **提取 CSS 变量**：从 `#wemd { ... }` 块解析 46 个 CSS 变量定义
...
`reference/dom-structure.md` 是组件 DOM 结构的唯一权威基准，由 `scripts/extract-dom-snapshot.mjs` 从主程序真源（`componentElements.ts` + `magazineRenderers.ts`）自动生成。写 CSS / 校验选择器前必须查阅，**禁止手工维护**。
...
- `output/theme-package/manifest.json` — 主题包核心配置文件
- `output/theme-package/brand.md` — 品牌描述文档
- `output/theme-package/styles/components.css` — 完整 CSS 样式
- `output/{theme-name}.wemd-theme` — 最终压缩包
