...
用户输入的品牌信息（名称、类型、人格、介绍、关键词等）。若用户提供了 Logo/头像的气质描述（如"圆润曲线、暖色调、科技感"），或对话中附了图片，据此提炼 `existing_assets`；否则 `existing_assets` 标注未提供即可，不强求。
...
- `existing_assets`（可选）：从用户提供的视觉气质描述或对话附图中分析已有视觉特征（几何结构、线条、色调、质感）。
...
- `existing_assets`（可选）→ 创作者已有的视觉偏好（若提供头像/主页/样张描述则分析其几何、线条、色调、质感）。
