...
function buildAudienceHint(audience: Audience): string {
  ...
  const layoutMap: Record<string, string> = {
    ...
    learning:
      "多用 callout-pro 标记重点，章节用原生 #### 小标题，必要时用 faq 或 resource-list 分步",
    decision:
      "数据用原生表格呈现、callout-pro 标记结论，步骤/清单用 resource-list",
    ...
    constraints.designGoal === "reading"
      ...
      : constraints.designGoal === "visual"
        ...
        : constraints.designGoal === "infoDensity"
          ? "优先信息表达效率。多用原生表格、列表、引用和杂志层组件，组件服务于信息传达而非装饰。"
          : "在阅读体验和视觉表现之间取得平衡。";
...
export function buildTemplatePrompt(
  ...
  return [
    ...
    "- 数据型段落后适合紧跟 callout-pro 做可视化强调，或用原生表格直接呈现数据",
    ...
    "",
    "### 标题（基础层，无需你处理）",
    "- 以 `#/##/###` 开头的 Markdown 标题段由系统基础层**自动、确定性**转换为标题组件，你不要也不应该输出 section-title 之类的标题组件",
    "- 不要在 content 里手工制造标题、也不要尝试把标题段引用进 article-section 之外的组件",
    "- 标题段是否出现在正文里，由基础层统一决定，你无需关心",
    "",
    "### 过渡信号",
    ...
    "- 文章内部章节目录由基础层原生标题自动生成，不要用 resource-list 造目录，它只用于外部资源/工具/步骤",
    ...
    "- 这是有序的系列连载，不要拿它生成普通的相关文章散列（相关推荐由基础层处理）",
    ...
    "- evidence — 数据/引用论据（通常是 quote-card / callout-pro）",
    ...
    "   - 数据密集段落 → 紧接 callout-pro，或改用原生表格直接呈现数据",
    "   - 金句段落 → 紧接 quote-card 或 full-quote",
    "   - 章节切换 → section-divider，章节小标题用原生 ####",
    ...
    "## 内容提炼规则",
    "",
    "1. hero-banner / magazine-cover 等标题类组件：精炼原文，不要照抄",
    "2. quote-card：从原文提取最精彩的一句话作为金句，author 从原文找（没有就留空）",
    "3. callout-pro：从原文提炼要点/提示/警告，type 根据语气选 tip/warning/danger/info",
    "4. faq：从原文提炼 2-3 个读者可能问的问题和答案",
    "5. 所有组件内容长度控制：单个组件 ≤ 200 字，避免冗长",
    "6. 严禁虚构原文没有的内容（如编造作者、编造数据、编造用户评价）",
    "",
    "## 示例",
    "",
